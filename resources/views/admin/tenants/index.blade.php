<x-admin-layout title="{{ __('Tenants') }}">
    <x-slot:actions>
    ...
    @if ($tenants->isEmpty())
        <div class="flex flex-col items-center gap-3 rounded-2xl bg-white py-16 text-center shadow-sm">
            ...
            <p class="text-sm font-medium text-ink-900">{{ __('No tenants yet') }}</p>
            <p class="text-sm text-gray-400">{{ __('Add a tenant and its admin.') }}</p>
            <a href="{{ route('admin.tenants.create') }}"
    ...
    @else
        <div class="overflow-hidden rounded-2xl bg-white shadow-sm">
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-100">
                            ...
                            <th class="px-5 py-3.5 text-right text-xs font-medium uppercase tracking-wide text-gray-400">{{ __('Admin') }}</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @foreach ($tenants as $tenant)
                            <tr class="group transition-colors duration-100 hover:bg-gray-50/60">
                                <td class="px-5 py-4 font-medium text-ink-900">{{ $tenant->name }}</td>
                                <td class="px-5 py-4 text-gray-600">{{ $tenant->owner?->name ?? '—' }}</td>
                                <td class="px-5 py-4 text-gray-600">{{ number_format($tenant->users_count) }}</td>
                                <td class="px-5 py-4 text-gray-600">{{ number_format($tenant->projects_count) }}</td>
                                <td class="px-5 py-4">
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-1 opacity-0 transition-opacity duration-150 group-hover:opacity-100">
                                        ...
                                        <form method="POST" action="{{ route('admin.tenants.destroy', $tenant) }}"
                                              onsubmit="return confirm('{{ __('Delete tenant :name and all its users? This action cannot be undone.', ['name' => $tenant->name]) }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
            ...
            @if ($tenants->hasPages())
                <div class="border-t border-gray-100 px-5 py-4">{{ $tenants->links() }}</div>
            @endif
        </div>
    @endif
</x-admin-layout>
